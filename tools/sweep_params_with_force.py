...
def run_single_game(cli_path, weights_path, seed, search_settings, config):
    try:
        ...
        temp_snapshot_path = temp_dir / f"temp_param_snapshot_{seed}.json"
    ...
    env["HARMONIES_FORCE_SPIRIT_LIMIT"] = "5.0" # Fixed Force 5 limit
...
def main():
    parser = argparse.ArgumentParser(description="Sweep heuristic parameters under Force 5 limit")
    ...
    scenarios = [
        ("Base (Mult 2.5, Penalty -4.0, Comp -0.05)", {"prox_mult": 2.5, "penalty_coeff": -4.0, "comp_coeff": -0.05}),
        ("Comp Heavy (Mult 2.5, Penalty -4.0, Comp -0.10)", {"prox_mult": 2.5, "penalty_coeff": -4.0, "comp_coeff": -0.10}),
        ("Penalty Heavy (Mult 2.5, Penalty -8.0, Comp -0.05)", {"prox_mult": 2.5, "penalty_coeff": -8.0, "comp_coeff": -0.05}),
        ("Blended (Mult 3.0, Penalty -6.0, Comp -0.08)", {"prox_mult": 3.0, "penalty_coeff": -6.0, "comp_coeff": -0.08}),
        ("Stepwise Only (Mult 2.5, Penalty -8.0, Comp 0.0)", {"prox_mult": 2.5, "penalty_coeff": -8.0, "comp_coeff": 0.0}),
        ("Extreme Comp (Mult 2.5, Penalty -4.0, Comp -0.15)", {"prox_mult": 2.5, "penalty_coeff": -4.0, "comp_coeff": -0.15})
    ]
    
    seeds = list(range(101, 101 + args.games))
    summary = []

    print(f"Sweeping parameter configs under Force 5 turns limit over {args.games} games per config.\n")
    ...
    print("\n================ PARAM SWEEP (WITH FORCE 5) RESULTS ================")
