...
def run_single_game(cli_path, weights_path, seed, search_settings, force_spirit_limit):
    try:
        ...
        temp_snapshot_path = temp_dir / f"temp_forced_snapshot_{seed}.json"
    ...
    if force_spirit_limit is not None:
        env["HARMONIES_FORCE_SPIRIT_LIMIT"] = str(force_spirit_limit)
...
def main():
    parser = argparse.ArgumentParser(description="Test forced spirit card completion")
    parser.add_argument("--games", type=int, default=20, help="Number of games per config")
    ...
    print(f"Testing forced spirit completion over {args.games} games per config with {args.workers} workers.\n")
    ...
    for name, limit in scenarios:
        ...
        with ThreadPoolExecutor(max_workers=args.workers) as executor:
            futures = {
                executor.submit(run_single_game, cli_path, DEFAULT_WEIGHTS, seed, settings, limit): seed
                for seed in seeds
            }
    ...
    print("\n================ FORCED SPIRIT COMPLETION RESULTS ================")
